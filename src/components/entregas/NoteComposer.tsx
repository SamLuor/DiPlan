...
export function NoteComposer({ entregaId }: { entregaId: string }) {
  ...
  const [autor, setAutor] = useState('Usuário Teste')
  const [texto, setTexto] = useState('')
  const [proximoPasso, setProximoPasso] = useState('')
  const [anexoNome, setAnexoNome] = useState<string | null>(null)

  const mutation = useMutation({
    mutationFn: () => addNotaFn({ data: { entregaId, texto, autor, proximoPasso, anexoNome } }),
    onSuccess: () => {
  ...
  return (
    <div className="flex flex-col gap-2 border-t px-6 pt-3.5 pb-5">
      <Input value={autor} onChange={(e) => setAutor(e.target.value)} placeholder="Seu nome" />
      ...
      <div className="flex items-center justify-between gap-2.5">
        <label htmlFor="registro-anexo-input" className="flex cursor-pointer items-center gap-1.5 text-xs text-primary">
          ...
          {anexoNome || 'Anexar arquivo'}
        </label>
        <input id="registro-anexo-input" type="file" className="hidden" onChange={(e) => setAnexoNome(e.target.files?.[0]?.name ?? null)} />
